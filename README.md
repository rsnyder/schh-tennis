...
- `GET /` — phone-optimized page: facility tabs (South / North / West), a vertical time-slot list with full player names, open-court summaries, day navigation (‹ ›), and a "Now" highlight that auto-scrolls to the current slot. This is the page the signage QR code points to. It's an installable PWA (manifest + minimal service worker + app icons at `/manifest.webmanifest`, `/sw.js`, `/icons/*`): "Add to Home Screen" gives an app icon, splash screen, and standalone (no browser chrome) presentation. Data always comes from the network; the service worker only keeps the shell available offline.
